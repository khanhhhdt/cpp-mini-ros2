...
int main()
{
    ...
    // CALLBACK GROUPS
    // ==================================================
    auto exclusiveGroup = node.createCallbackGroup(CallbackGroupType::MutuallyExclusive);
    auto reentrantGroup = node.createCallbackGroup(CallbackGroupType::Reentrant);
    ...
    auto posePublisher = node.createPublisher<Pose2D>("/pose");

    // ==================================================
    // SIMPLE LAMBDA
    // ==================================================

    auto lambdaSub = node.createSubscriber<LaserScan>("/scan",
                                                      exclusiveGroup,
                                                      [](std::shared_ptr<const LaserScan> scan)
                                                      {
                                                          std::cout
    ...
    auto freeFunctionSub = node.createSubscriber<LaserScan>(
        "/scan",
        reentrantGroup,
    ...
    auto memberSub = node.createSubscriber<LaserScan>("/scan",
                                                      &Mapper::onScan,
                                                      mapper);

    auto poseSub = node.createSubscriber<Pose2D>("/pose",
                                                 &Mapper::onPose,
                                                 mapper);
    ...
    {
        auto tempMapper = std::make_shared<Mapper>();

        auto tempSub = node.createSubscriber<LaserScan>("/scan",
                                                        &Mapper::onScan,
                                                        tempMapper);

        std::cout << "[TEMP MAPPER CREATED]"
                  << std::endl;
    }

    std::cout << "[TEMP MAPPER DESTROYED]"
              << std::endl;
    ...
    auto serviceHandle = node.createService<AddTwoIntsRequest,
                                            AddTwoIntsResponse>("/add_two_ints",
    ...
    auto client = node.createClient<AddTwoIntsRequest,
                                    AddTwoIntsResponse>("/add_two_ints");
    ...
    auto request = std::make_shared<AddTwoIntsRequest>();
    ...
    Timer scanTimer(std::chrono::milliseconds(1000),
                    [&]()
                    {
                        auto scan =
    ...
    Timer poseTimer(std::chrono::milliseconds(1500),
                    [&]()
                    {
                        auto pose = std::make_shared<Pose2D>();
    ...
    Timer unsubscribeTimer(std::chrono::milliseconds(5000),
                           [&]()
                           {
                               std::cout << "\n[UNSUBSCRIBE LAMBDA]"
                                         << std::endl;
